package me.soda.moonhud.meteor.hud;
...
public class MoonHud extends HudElement {
    ...
    private final Setting<Boolean> weather = sgGeneral.add(new BoolSetting.Builder()
        .name("weather")
    ...
    public void tick(HudRenderer renderer) {
        ...
        if (!Utils.canUpdate()) {
            ...
            if (weather.get()) {
                if (mc.world.isThundering()) weatherText = "Weather: Thunder";
                else if (mc.world.isRaining()) weatherText = "Weather: Rain";
                else weatherText = "Weather: Sun";
            }
        }

        w += renderer.textWidth(nextPhase, shadow.get(), getScale()); //This is the longest text
        h = Math.max(h, renderer.textHeight(shadow.get(), getScale()) * (weather.get() ? 3 : 2));
        setSize(w, h);
    }
    ...
    public void render(HudRenderer renderer) {
        ...
        if (weather.get()) {
            y += renderer.textHeight(shadow.get(), getScale());
            renderer.text(weatherText, x, y, Color.WHITE, shadow.get(), getScale());
        }
    }
